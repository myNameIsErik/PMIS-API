...
use App\Models\Chart2022;
use Illuminate\Http\Request;

class Chart2022Controller extends Controller
{
    public function all(Request $request)
    {
        ...
        if ($id)
        {
            $chart = Chart2022::all()->find($id);

            if ($chart) {
                return ResponseFormatter::success(
                    $chart,
                    'Data Chart 2022 Berhasil diambil.'
                );
            } else {
                return ResponseFormatter::error(
                    null,
                    'Data Chart 2022 Tidak Ada.',
                    404
                );
            }
        }
